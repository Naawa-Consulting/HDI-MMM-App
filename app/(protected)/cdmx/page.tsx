...
import { ModelOverviewChart } from "../nacional/ModelOverviewChart";
import { fmtPct, fmtNum, RUN_TYPE_LABELS } from "@/lib/utils";

export const revalidate = 300;

export default async function CdmxPage() {
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        <div className="flex items-center gap-3 flex-wrap">
          <h1 className="text-gray-900 text-2xl font-semibold">Modelo CDMX</h1>
          ...
          <div className="space-y-2">
            {blocks.map((b) => (
              <div key={b.id} className="flex items-center gap-3">
                <span className="text-gray-500 text-xs w-32 text-right capitalize">{b.block}</span>
                <div className="flex-1 h-3 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    className="h-full rounded-full"
                    style={{
                      width: `${Math.max(0, Math.min(100, b.pct))}%`,
                      backgroundColor: b.block === "marketing" ? "#65A518" : "#00A3A8",
                      opacity: b.block === "marketing" ? 1 : 0.5 + blocks.indexOf(b) * 0.1,
                    }}
                  />
                </div>
                <span className="text-gray-700 text-xs font-medium w-12">{fmtPct(b.pct, 1)}</span>
              </div>
            ))}
          </div>
        </div>
